...
export type ExpensePoint = { day: string; expense: number };

function clamp(n: number, a: number, b: number) {
  return Math.max(a, Math.min(b, n));
}

export function FinanceExpensesChart({ series }: { series: ExpensePoint[] }) {
  const w = 720;
  const h = 240;
  const padL = 44;
  const padR = 16;
  ...
  const max = Math.max(1, ...series.map((d) => d.expense));
  ...
  function handleMove(e: React.MouseEvent<SVGSVGElement>) {
    ...
    const top = y(series[i].expense) * (rect.height / h);
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <div className="text-sm font-semibold text-foreground">Daily expenses (30 days)</div>
          <div className="text-xs text-muted-foreground">Outflow only</div>
        </div>
        ...
        {hp && tipPos && (
          ...
          >
            ...
            <div className="mt-1 text-xs text-foreground">
              Expense: <span className="font-semibold">{money(hp.expense)}</span>
            </div>
          </div>
        )}
      </div>
